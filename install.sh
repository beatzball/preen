...
# glow 2 throws away color when its output is a pipe, which is exactly what fzf
...
if have glow && ! { [ "$(glow_major)" -ge 3 ] 2>/dev/null; }; then
  if [ "$NO_DEPS" = 1 ]; then
    warn "glow $(glow_major) is too old for colored previews; need 3 or newer"
  else
    warn "glow $(glow_major) drops color in pipes; upgrading to 3 or newer"
    case "$PKG" in
    ...
    { [ "$DRY" = 1 ] || { [ "$(glow_major)" -ge 3 ] 2>/dev/null; }; } \
      ...
      || err "still on glow $(glow_major); previews will have no color"
  fi
fi
